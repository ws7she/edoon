 ...
 */
    ...
    var
      remoteUserIdMsg = $.validator.format('该账号已注册');
    $.validator.addMethod('remoteUserId', function (value, element, param) {
      var $this = this;
      var isuserid = $.validator.methods['remote'].call(this, value, element, {
         url: "#RPL:checkUserId#",
         type: "post",
         dataType: "json",
         dataFilter: function (data) {　　　　//判断控制器返回的内容
             if(typeof data === "string"){
               try {
                 ...
                 log(e, "校验注册用户邮箱或手机是否存在", "服务器返回数据JSON格式不正确");
               } finally {
             ...
             if (data.code == 0) {
                ...
                //错误提示信息用后端返回的
                 $this.settings.messages[element.name].remoteUserId = data.result;
                 $(element).attr("aria-message", data.result);
                 return false;
             }
         }
      });
      return this.optional(element) || isuserid;
    }, remoteUserIdMsg);

    $.validator.addMethod('phone_email', function (value, element, param) {
        var phone = $.validator.methods['phoneZH'].call(this, value, element, param),
            email = $.validator.methods['email'].call(this, value, element, param);
